...
//                    // 상품 정보를 화면에 표시
//                    val txtList = keepInfoList?.map { keepInfo ->
//                        "상품명 : ${keepInfo?.kpName}\n" +
//                                "수량 : ${keepInfo?.kpCnt}\n" +
//                                "정가 : ${keepInfo?.kpJeongGa}\n" +
//                                "판매가 : ${keepInfo?.kpPrice}\n\n"
//                    }
//
//                    // 리스트를 문자열로 변환하여 TextView에 설정
//                    binding.tv.text = txtList?.joinToString("")
        ...
        binding.btnOk.setOnClickListener {
            ...
            // 여기에 체크된 아이템을 데이터베이스에 저장하는 코드 추가
            // checkedItems 리스트에는 체크된 아이템들이 들어있습니다.
            // 예시: Retrofit을 사용한 API 호출 등의 방식으로 데이터베이스에 저장

            RetrofitBuilder.api.keepInsert(checkedItems).enqueue(object : Callback<Int> {
